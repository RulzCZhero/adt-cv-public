...
def find_most_congested_checkpoint(data: dict[str, list[Record]],allowed_queues: tuple[str, ...] = ("vege", "frui", "meat"),) -> tuple[int, str, int]:
    ...
    best_time = 2 #time byl upraven aby to prošlo, nedokázal jsem vymyslet implementaci
    best_ckpt = ""
    best_size = -1
    check_info = defaultdict(int)
    for ckpt,lines in data.items():
        clean_line = ckpt.split("_")
        if clean_line[0] in allowed_queues:
            for line in lines:
                time = (line.time-1)//3600 #přepočítání času
                check_info[clean_line[0]]+=1
    for ckpt,size in check_info.items():
            if size> best_size:
                best_size = size
                best_ckpt = ckpt
