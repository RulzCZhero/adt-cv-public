#Tenhle practice vychází z practice10.py, jde o zakomponování dataclass
...
def main():
    ...
    #práce s dataclass
    students = []
    with open(analytic,"r",encoding="utf-8") as f:
        for student in f:
            temp = student.split(";")
            if float(temp[2])>=50:
                tempStudent = Student(temp[1],temp[0],float(temp[2]),True)
            else:
                tempStudent = Student(temp[1],temp[0],float(temp[2]),False)      
