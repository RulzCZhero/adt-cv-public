...
def compute_hourly_usage_per_checkpoint(data: dict[str, list[Record]]) -> dict[str, list[int]]:
    ...
    """
    stats: dict[str, list[int]] = {ckpt: [0] * 24 for ckpt in data}

    for ckpt, records in data.items():
        for record in records:
            hour_index = (record.time - 1) // 3600
            
            
            if 0 <= hour_index < 24:
                stats[ckpt][hour_index] += 1
...
def find_most_used_checkpoint_at_hour(hourly_result: dict[str, list[int]], hour: int) -> str:
    ...
    for ckpt, hourly_list in hourly_result.items():
        
        if hourly_list[hour] > most_customers:
            most_customers = hourly_list[hour]
